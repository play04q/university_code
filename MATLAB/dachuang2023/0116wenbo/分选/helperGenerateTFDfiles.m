function helperGenerateTFDfiles(parentDir,dataDir,wav,truth,Fs)   % wav信号数据  truth调制类型  Fs采样频率
% This function is only intended to support
% ModClassificationOfRadarAndCommSignalsExample. It may change or be
% removed in a future release.
    
[~,~,~] = mkdir(fullfile(parentDir));  %是用来创建文件夹的。其中 parentDir 是父文件夹的路径，
...
for idxM = 1:length(modTypes)   %用于创建一些文件夹。其中 modTypes 是一个包含唯一值的向量或矩阵，表示数据集中所有可能的分类或标签。循环将遍历所有这些分类，并为每个分类创建一个文件夹。
    ...
    [~,~,~] = mkdir(fullfile(parentDir,char(modType)));
...
for idxW = 1:length(truth)
   ...
   TFD = imresize(TFD,[227 227]);  %调整时频图像的大小为 227 x 227。
   TFD = rescale(TFD);    %将时频图像的像素值归一化到 [0,1] 的范围内
   modType = truth(idxW);
   
   imwrite(TFD,fullfile(parentDir,char(modType),sprintf('%d.png',idxW)))   
