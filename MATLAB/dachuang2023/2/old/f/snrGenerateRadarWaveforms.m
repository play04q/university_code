function [data, truth] = snrGenerateRadarWaveforms(shuliang,snr)
%%%%%%%%%%%%信号初始数据%%%%%%%%%%%%%%%%%%%
Fs = 500e6;        %采样频率
 nSignalsPerMod = shuliang ;    %产生信号个数
...
for iM = 1:length(modTypes)
    modType = modTypes(iM);
    rangeFc = [Fs/10, Fs/4];    %载频范围
    rangeN = [512, 1920];     %产生信号点数范围
    %snrVector = -10:2:10;    %信噪比-10:10
    SNR= snr;
    
  
    
    
    rangeB = [Fs/40, Fs/16];     %LFM信号带宽范围
