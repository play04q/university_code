...
freq = 3e+9; % radar operating fiequency in Hz
g= 34.5139; % antenna gain in dB
sigmam =0.5; % missile RCS' m squared
sigmaa =4;%aireraf RCS m squared
...
% compute the improvement/actor due to 7-pulse non-coherent integration 
Improv = improv_fac (np, pfa, pd);
% calculate the integration loss 
lossnci = 10*log10(np) -Improv; 
% calculate net gain in SNR due to integration 
...
SNR_7_pulse_NCI_aircraft=SNR_single_pulse_aircraft + SNR_net;
disp(['SNR_single_pulse_missile=',num2str(SNR_single_pulse_missile)]);
disp(['SNR_7_pulse_NCI_missile=',num2str(SNR_7_pulse_NCI_missile)]);
disp(['SNR_single_pulse_aircraft=',num2str(SNR_single_pulse_aircraft)]);
disp(['SNR_7_pulse_NCI_aircraft=',num2str(SNR_7_pulse_NCI_aircraft)]);