...
switch quest
    case 1  %%计算结果及压力分布, 单点求解计算
        ...
        figure(7)
        set(7,'Name','数值计算结果','NumberTitle','on')
        axis off
        text(-.1,1,'数值计算结果 ')
        text(-.1,.95,'JID: '); text(0.15,0.95,data)
        text(0.3,.95,'Downwash matrix condition: '); text(0.7,0.95,num2str(results.dwcond))
        
        text(-.1,.90,'参考面积 [m^2]: ');	text(0.15,0.90,num2str(ref.S_ref));	
        text(-.1,.85,'平均气动弦长 [m]:  ');text(0.2,.85,num2str(ref.C_mac));%%Reference chord
        text(-.1,.8,'展长 [m]: ');text(0.15,.8,num2str(ref.b_ref));  %%Reference span
        
        text(0.35,.85,'参考点坐标 [m]: ');text(0.65,.85,num2str(geo.ref_point));
        text(0.35,.80,'重心坐标   [m]: ');text(0.65,.80,num2str(geo.CG));
        
        text(0,.7,'Net Wind Forces: (N)');			
        text(0.0,.65,'升力: ');		text(0.1,.65,num2str(results.L));	
        text(0.0,.6,'阻力: ');		text(0.1,.6,num2str(results.D));
        text(0.0,.55,'侧力: ');		text(0.1,.55,num2str(results.C));
        ...
        text(0,.2,'STATE: ');
        text(0,.15,'alpha: '); text(.15,.15,num2str(state.alpha*180/pi));
        text(0,.1,'beta: '); text(.15,.1,num2str(state.beta*180/pi));
        ...
        text(0.3,.15,'P: '); text(.45,.15,num2str(state.P));
        text(0.3,.10,'Q: '); text(.45,.10,num2str(state.Q));
        text(0.3,.05,'R: '); text(.45,.05,num2str(state.R));
        ...
        text(0.4,.85,'\beta: '); 		text(.55,.85,num2str(state.beta*180/pi));
        ...
        axis off
        %grid on
        text(0,1,'数值计算结果, Central difference, 操纵导数')
        text(0,.95,'JID: '); text(0.25,0.95,data)
  ...
  case 2
      try  %%读取计算结果文件
       ...
       % d=size(z,2);
        ...
        set(4,'Name','ΔCp压力云图','NumberTitle','on')
        rotate3d on 
        colormap(hot);
        AAA=results.cp'; %#ok<NODEF>
        fill3(lattice.XYZ(:,:,1)',lattice.XYZ(:,:,2)',lattice.XYZ(:,:,3)',AAA)
        title('ΔCp分布云图')
        colorbar('vert')
        axis equal
        ...
        figure(10)
        set(10,'Name','机翼展向分布载荷','NumberTitle','on')
        hold on, grid on
        plot(results.ystation(:,1),results.ForcePerMeter(:,1));
        title('#1机翼展向分布载荷');
        ylabel('单位长度分布载荷, q, [N/m]');
        xlabel('展向位置, y, [m]')

        %Local CL plot
        figure(11)
        set(11,'Name','机翼展向升力分布','NumberTitle','on')
        hold on, grid on
        plot(results.ystation(:,1),results.CL_local(:,1));
        title('#1机翼展向升力分布');
        ylabel('升力系数 CL');
        xlabel('展向位置, y, [m]') 
        s=max(results.CL_local(:,1));
        text(0,s(1)+.01,strcat('全机升力系数 CL: ',num2str(results.CL)));

        figure(12)
        set(12,'Name','机翼展向升力分布(归一化)','NumberTitle','on')
        hold on, grid on
        plot(results.ystation(:,1)./(ref.b_ref/2),results.CL_local(:,1)./results.CL);
        title('#1机翼展向升力分布（归一化）');
        ylabel('相对升力系数 CL/CLW');
        xlabel('相对展向位置 y/b')

        %%计算剪力弯矩
        F0=results.ForcePerMeter(:,1);
        y=results.ystation(:,1);
        k=size(y);
        %%剪力计算部分
        Fs(1)=0;
        for i=2:k(1)/2   %左半翼展 Port Wing
           Fs(i)=Fs(i-1)+.5*(F0(i-1)+F0(i))*(y(i)-y(i-1));
        end
        Fs(k(1))=0;
        for i=k(1)-1:-1:k(1)/2+1
           Fs(i)=Fs(i+1)+.5*(F0(i+1)+F0(i))*(y(i)-y(i+1));
        end
        %%弯矩计算部分
        m(1)=0;
        for i=2:k(1)   %左半翼展 Port Wing
           m(i)=m(i-1)+.5*(Fs(i-1)+Fs(i))*(y(i)-y(i-1));
        end

       figure(13)
       set(13,'Name','机翼弯矩图','NumberTitle','on')
       hold on, grid on
       title('#1机翼展向弯矩分布');   
       ylabel('弯矩, M_b, [Nm]');
       xlabel('展向位置, y, [m]')     
       ...
       figure(14)
       set(14,'Name','机翼剪力图','NumberTitle','on')
       plot(y, Fs) ;
       hold on, grid on
       title('#1机翼展向剪力分布');   
       ylabel('剪力, F_s, [N]');
    ...
    figure(15)
    X=results.alpha_sweep*180/pi;
    
    subplot(3,2,1), plot(X,squeeze(results.matrix(1,1,:)));
    xlabel('Alpha [deg]')
    ylabel('CL')
   
    subplot(3,2,3), plot(X,squeeze(results.matrix(2,1,:)));
    xlabel('Alpha [deg]')
    ylabel('CD')
    
    subplot(3,2,5), plot(X,squeeze(results.matrix(5,1,:)));
    xlabel('Alpha [deg]')    
    ylabel('Cm')
    
    subplot(3,2,2), plot(X,squeeze(results.matrix(4,1,:)));
    xlabel('Alpha [deg]')
    ylabel('Cl')

    subplot(3,2,4), plot(X,squeeze(results.matrix(6,1,:)));
    xlabel('Alpha [deg]')
    ylabel('Cn')
    
    subplot(3,2,6), plot(X,squeeze(results.matrix(3,1,:)));
    xlabel('Alpha [deg]')
    ylabel('Cc')
...
case 4
   ...
   subplot(3,2,1), plot(results.beta_sweep,squeeze(results.matrix(1,1,:)));
   xlabel('Beta [rad]')
   ylabel('CL')
   
   subplot(3,2,3), plot(results.beta_sweep,squeeze(results.matrix(2,1,:)));
   xlabel('Beta [rad]')
   ylabel('CD')
   
   subplot(3,2,5), plot(results.beta_sweep,squeeze(results.matrix(3,1,:)));
   xlabel('Beta [rad]')
   ylabel('CY')
   
   subplot(3,2,2), plot(results.beta_sweep,squeeze(results.matrix(4,1,:)));
   xlabel('Beta [rad]')
   ylabel('Cl')
   
   subplot(3,2,4), plot(results.beta_sweep,squeeze(results.matrix(5,1,:)));
   xlabel('Beta [rad]')
   ylabel('Cm')
   
   subplot(3,2,6), plot(results.beta_sweep,squeeze(results.matrix(6,1,:)));
   xlabel('Beta [rad]')
   ylabel('Cn')
...
case 7
   ...
   subplot(3,2,3), plot(results.Q_sweep,squeeze(results.matrix(1,2,:)));
   xlabel('Q [-]')
   ylabel('CD [-]')
   
   subplot(3,2,5), plot(results.Q_sweep,squeeze(results.matrix(1,3,:)));
   xlabel('Q [-]')
   ylabel('CY [-]')
   
   subplot(3,2,2), plot(results.Q_sweep,squeeze(results.matrix(1,4,:)));
   xlabel('Q [-]')
   ylabel('Cl [-]')
   
   subplot(3,2,4), plot(results.Q_sweep,squeeze(results.matrix(1,5,:)));
   xlabel('Q [-]')
   ylabel('Cm [-]')
   
   subplot(3,2,6), plot(results.Q_sweep,squeeze(results.matrix(1,6,:)));
   xlabel('Q [-]')
   ylabel('Cn [-]')   
